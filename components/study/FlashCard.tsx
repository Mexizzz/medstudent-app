    ...
    <div className="space-y-5">
      ...
      >
        <div className={cn('flashcard-inner w-full h-full', flipped && 'flipped')}>
          {/* Front */}
          <div className="flashcard-front rounded-xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-slate-50 flex flex-col items-center justify-center p-6 text-center">
            <p className="text-xs font-semibold text-blue-400 uppercase tracking-wide mb-3">Tap to reveal</p>
            <p className="text-lg font-medium text-slate-800 leading-relaxed">{question.front}</p>
          </div>
          {/* Back */}
          <div className="flashcard-back rounded-xl border-2 border-emerald-200 bg-gradient-to-br from-emerald-50 to-slate-50 flex flex-col items-center justify-center p-6 text-center">
            <p className="text-xs font-semibold text-emerald-500 uppercase tracking-wide mb-3">Answer</p>
            <p className="text-base text-slate-800 leading-relaxed">{question.back}</p>
          </div>
        </div>
      </div>

      {!flipped && (
        <p className="text-center text-sm text-slate-400">Click the card to flip it</p>
      )}

      {flipped && !rated && (
        <div className="space-y-2">
          <p className="text-center text-sm text-slate-500 font-medium">How well did you know this?</p>
