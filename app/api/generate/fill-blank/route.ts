...
import { generateFillBlanks, mapGeneratorError } from '@/lib/ai/generators';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hasFeature(tier, 'fill_blank')) {
      return NextResponse.json({ error: 'Fill in the Blank is available on Pro and Max plans', upgradeRequired: true, requiredTier: 'pro' }, { status: 403 });
    }
