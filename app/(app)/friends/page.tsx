  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-slate-500">{friends.length} friend{friends.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Add Friend */}
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h2 className="text-sm font-semibold text-slate-700 mb-3">Add Friend by Username</h2>
        <div className="flex gap-2">
          <div className="flex-1 flex items-center border border-slate-200 rounded-lg px-3">
      ...
      {incoming.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-3">Friend Requests ({incoming.length})</h2>
              ...
              <div key={req.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div>
                  ...
                  {req.fromUser?.username && (
                    <p className="text-xs text-slate-400">@{req.fromUser.username}</p>
                  )}
                </div>
                <div className="flex gap-2">
                  <button
      ...
      {friends.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-slate-500">No friends yet. Add someone by username!</p>
        </div>
      ) : (
          ...
          {friends.map(friend => (
            <div key={friend.id} className="flex items-center justify-between p-4">
              <div className="min-w-0">
                ...
                {friend.bio && (
                  <p className="text-xs text-slate-500 mt-0.5 truncate">{friend.bio}</p>
                )}
              </div>
              <button
